...
//!
//! This is the slow-but-correct reference implementation of the exact WGSL
//! semantics in `expand.wgsl` plus the host-side merge step, kept deliberately
//! simple so the GPU path can be differentially tested against it (the
//! "correctness oracle first" rule: never optimize against an unverified
//! implementation). It mirrors the inner-ladder Hermite recursion of
//! `unfer/nested_fock_algebra`:
//!
//! ```text
...
//! ```
//!
//! All arithmetic is `f32` to match the WGSL shader bit-for-bit at the
//! tolerance used by the differential tests.

use crate::types::{HermiteState, OperatorTerm};

/// Apply one [`OperatorTerm`] to one state (the WGSL `apply_recursion` body).
///
/// Returns `None` for a "dead" state (annihilation on the vacuum: the shader
/// zeroes the amplitude and the host filters it out).
fn apply_term_to_state(state: HermiteState, op: &OperatorTerm) -> Option<HermiteState> {
    let mut out = state;
    let dim = op.target_dim as usize;
    if dim >= out.n.len() {
        // Out-of-range dimension: the shader indexes `state.n[dim]` (UB in
        // WGSL for dim >= 4); the reference treats it as identity so the
        // differential test can also cover malformed input.
        return Some(out);
    }
    let n_val = out.n[dim];
    ...
    // complex_mul(coeff, factor * multiplier) — mirrors the shader. Both
    // components are computed from the *original* coefficients (the shader
    // reads `state.coeff_re`/`state.coeff_im` before writing `output_states`).
...
/// The reference `apply_operator`: one pass per term, then merge-sort-reduce.
///
/// Mirrors `DeltaAlgebraEngine::apply_operator` (per-term monomial passes,
/// aggregation of identical states, drop of zero-amplitude states).
...
/// Merge-sort-reduce: sort by quantum numbers, sum identical states, drop
/// zero-amplitude states. Mirrors `DeltaAlgebraEngine::aggregate_states`.
...
mod tests {
    ...
    fn reference_merges_identical_states() {
        ...
        // 2·a†_0 |0> = 2|1,0> — both passes hit the same state, amplitudes sum.
        let double = [
            OperatorTerm::new(OpType::Creation, 0, 1.0, 0.0),
            OperatorTerm::new(OpType::Creation, 0, 1.0, 0.0),
        ];
