...
use typst_imaging::convert::{convert_fixed_stroke, convert_geometry};
use typst_imaging::paint::shape_paint;
use typst_library::visualize as viz;

pub fn render_shape(shape: &viz::Shape, sink: &mut impl KanvaSink, state: RenderState) {
    ...
    sink.draw_path(path, state.transform, fill, stroke, Default::default());
}
